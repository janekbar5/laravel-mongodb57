...
class BooksController extends Controller {
    ...
    public function autocomplete1(Request $request) {
        $data = Postcode::where("postcode", "LIKE", "%{$request->input('query')}%")->get();
    ...
    public function autocomplete(Request $request) {
        ...
        $queries = DB::collection('postcodes')
                        ->where('postcode', 'LIKE', '%' . $term . '%')
                        ->orWhere('location2', 'LIKE', '%' . $term . '%')
                        ->take(20)->get();

        foreach ($queries as $query) {
            $results[] = ['id' => $query['postcode'], 'value' => $query['postcode'] . ' ' . $query['location2']];
        }
